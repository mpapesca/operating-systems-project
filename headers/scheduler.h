...
int nextoperation(job_t *job) {
  // printout(log_file,"nextoperation()\n");
  ...
  if(op_current == NULL) {
    // printout(log_file,"JOB %d OPERATIONS FINISHED\n", job->id);
    return OP_FIN;
  }
 ...
 */
int longtermscheduler_fcfs(void) {
//TODO:Incorperate appendjobtolist()
...
int longtermscheduler_sjf(void) {
  job_t * queue_tail;
  reg_t * queue_reg;
  operation_t * queue_op;
  int found_waiting = 0;
      ...
      if( CURRENT_OS_RUNTIME >= current->arrival_time) {
        ...
        switch(nextoperation(current)) {
          case CPU_OP:
            ...
            prependjobcopytonode(READY, current, false);
            // if(READY_Q == NULL) {
            //   READY_Q = malloc(sizeof(job_t));
            //
            //   queue_tail = READY_Q;
...
int longtermscheduler_priority_pre(void) {

  return 0;
}

int longtermscheduler_priority_npre(void) {

  return 0;
}

int longtermscheduler_rr(void) {
...
void shorttermscheduler(void) { //Still needs a lot of work.
  ...
  // printout(log_file,"shorttermscheduler()\n");
  ...
  if(READY_Q->process_state == READY) {
    ...
    if(op_current->status == 0) {
      // printout(log_file,"NEW CPU OPERATION\n");
